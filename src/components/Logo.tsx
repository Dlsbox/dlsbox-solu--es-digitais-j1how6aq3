...
export function Logo({ className }: { className?: string }) {
  return (
    <Tilt
      className={cn('logo-card rounded-2xl cursor-pointer', className)}
      maxRotation={12}
      glareEnable
      glareMaxOpacity={0.15}
    >
      <div className="flex items-center gap-3 bg-[#060811] pl-3 pr-5 py-2.5 rounded-[inherit] shadow-lg border border-white/10 select-none w-full h-full">
        {/* Isometric Cube */}
        <div className="relative w-8 h-8 flex-shrink-0 group [perspective:1000px] ml-1">
          <div
